...
function dato(doc, x, y, label, value, w) {
  doc.setFont("helvetica", "bold"); doc.setFontSize(4.9); doc.setTextColor(...SOFT); doc.text(label, x, y);
  doc.setFontSize(7.2); doc.setTextColor(...INK); doc.text(doc.splitTextToSize(val(value), w)[0], x, y + 2.9);
}
...
export function generarReporteMateriales({ caso = {}, orden = {} }) {
  ...
  doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.setTextColor(...INK); doc.text("REPORTE DE MATERIALES", M, 8);
  doc.setFontSize(7); doc.setTextColor(...SOFT); doc.text("PARA SUMINISTROS", M, 11.5);
  doc.setFont("helvetica", "normal"); doc.setFontSize(5.8); doc.text(`Fecha de impresion: ${new Date().toLocaleDateString("es-DO")}`, M, 14.5);
  doc.setFont("helvetica", "bold"); doc.setFontSize(6); doc.text("LLAVE", W - M, 6.5, { align: "right" }); doc.setFontSize(15); doc.setTextColor(...RED); doc.text(caso.numero_llave ? `#${caso.numero_llave}` : "-", W - M, 13, { align: "right" });
  ...
  doc.setFillColor(...INK); doc.rect(M, y, CW, 5.8, "F"); doc.setFont("helvetica", "bold"); doc.setFontSize(6.5); doc.setTextColor(255, 255, 255); headers.forEach((h, i) => doc.text(h, xs[i] + 2, y + 3.9)); y += 5.8;
  const rowH = 5.7;
  doc.setDrawColor(...LINE); doc.setLineWidth(0.25); doc.setFont("helvetica", "normal"); doc.setFontSize(7.3); doc.setTextColor(...INK);
  ...
  y += 3;
  doc.setFont("helvetica", "bold"); doc.setFontSize(8.5); doc.setTextColor(...INK); doc.text("TOTAL DE MATERIALES: RD$ __________________________________", M, y);
  y += 7.5;
