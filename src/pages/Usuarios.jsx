...
const empty = { nombre_completo: "", rol: "administracion_taller", especialidad: "", pin: "", activo: true };
...
export default function Usuarios() {
  ...
  return <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8"><div className="flex flex-wrap items-end justify-between gap-4 mb-7"><div><h1 className="text-2xl sm:text-3xl font-extrabold text-[var(--ink)]">Usuarios y accesos</h1><p className="text-sm text-[var(--ink-soft)] mt-1">Crea trabajadores, roles y PINs sin entrar a Supabase.</p></div><button className="btn-primary" onClick={() => setModal({ ...empty })}><Icon name="plus" className="w-5 h-5" /> Nuevo usuario</button></div>{error && <div className="card p-4 mb-5 text-sm text-[var(--brand-red)]">{error}</div>}{loading ? <p className="text-[var(--ink-soft)]">Cargando usuarios…</p> : <div className="grid md:grid-cols-2 gap-4">{usuarios.map((u) => <article key={u.user_id} className="card p-5 flex items-center gap-4"><span className="w-11 h-11 rounded-xl bg-[var(--brand-red-50)] text-[var(--brand-red)] flex items-center justify-center shrink-0"><Icon name="user" className="w-5 h-5" /></span><div className="min-w-0 flex-1"><p className="font-bold text-[var(--ink)] truncate">{u.nombre_completo}</p><p className="text-sm text-[var(--ink-soft)] truncate">{etiqueta(u.rol)}{u.especialidad ? ` · ${u.especialidad}` : ""}</p><span className={`inline-block mt-2 text-xs font-bold px-2 py-0.5 rounded-full ${u.activo ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}>{u.activo ? "Activo" : "Inactivo"}</span></div><div className="flex gap-1"><button onClick={() => setModal({ ...u, pin: "" })} className="p-2 text-[var(--ink-soft)] hover:text-[var(--brand-red)]" aria-label="Editar"><Icon name="pencil" /></button><button onClick={() => eliminar(u)} className="p-2 text-[var(--ink-soft)] hover:text-[var(--brand-red)]" aria-label="Eliminar"><Icon name="trash" /></button></div></article>)}{usuarios.length === 0 && <div className="card p-10 text-center text-[var(--ink-soft)] md:col-span-2">No hay usuarios creados con PIN todavía.</div>}</div>}{modal && <UsuarioModal usuario={modal} onClose={() => setModal(null)} onSaved={async () => { setModal(null); await cargar(); }} />}</div>;
}
