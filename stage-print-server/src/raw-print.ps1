...
public static class StageRawPrinter {
  ...
  public static void Send(string printerName, byte[] bytes) {
    ...
    try {
      var doc = new DOC_INFO_1 { pDocName = "STAGE AI LABS - Etiqueta", pDataType = "RAW" };
      ...
      try {
        ...
        try {
          ...
          if (!WritePrinter(printer, bytes, bytes.Length, out written) || written != bytes.Length)
            throw new System.ComponentModel.Win32Exception(Marshal.GetLastWin32Error(), "No se enviaron todos los datos.");
...
[StageRawPrinter]::Send($PrinterName, $bytes)
Write-Output "OK"
